"""Execute GitHub repository configuration from YAML."""
...
from github_management.git_service import GitError, GitService
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "add",
                    "origin",
                    f"https://github.com/{remote_config.get('owner', 'yogipatel5')}/{repo_name}.git",
                ],
                check=True,
            )
            ...
            if "already exists" in e.stderr:
                raise ConfigExecutionError(
                    f"Repository '{repo_name}' already exists and was not handled in pre-check. "
                    f"Please delete it first or choose a different name."
                )
