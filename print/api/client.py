...
class PrintBaseClient:
    """Base client for interacting with external services"""

    def __init__(self):
        if not os.getenv("PRINT_API_KEY"):
            raise ValueError("PRINT_API_KEY environment variable is required")
        self.base_url = os.getenv("PRINT_API_URL", "")
        self.headers = {"X-API-Key": os.getenv("PRINT_API_KEY"), "Content-Type": "application/json"}
        self.verify_ssl = os.getenv("PRINT_VERIFY_SSL", "true").lower() == "true"
