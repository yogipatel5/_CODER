...
This directory contains management commands for the print app. Commands are organized using a namespace system.
...
└── commands/
    ├── __init__.py
    ├── print.py     # Parent command (handles subcommands)
...
All commands in this app are namespaced under `print`. To run a command:

```bash
python manage.py print <subcommand> [options]
```
...
1. **setup**: Initialize app configuration and data
   ```bash
   python manage.py print setup [--force]
   ```
...
```bash
python manage.py print my-command --example "value"
```
