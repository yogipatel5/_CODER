"""
Show all auto-discovered models, tasks, and signals in the print app.
"""
...
class Command(BaseCommand):
    help = "Show all auto-discovered models, tasks, and signals in the print app"

    def _get_models(self):
        """Get all models from the app."""
        app_config = apps.get_app_config("print")
    ...
    def _get_tasks(self):
        ...
        return [
            (name, obj)
            for name, obj in inspect.getmembers(print.tasks)
            if inspect.isfunction(obj) and hasattr(obj, "delay")
        ]

    def _get_signals(self):
        """Get all signal handlers from the app."""
        import print.signals

        return [
            (name, obj)
            for name, obj in inspect.getmembers(print.signals)
