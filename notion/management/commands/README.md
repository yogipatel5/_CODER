...
This directory contains management commands for the notion app. Commands are organized using a namespace system.
...
└── commands/
    ├── __init__.py
    ├── notion.py     # Parent command (handles subcommands)
...
All commands in this app are namespaced under `notion`. To run a command:

```bash
python manage.py notion <subcommand> [options]
```
...
1. **setup**: Initialize app configuration and data
   ```bash
   python manage.py notion setup [--force]
   ```
...
```bash
python manage.py notion my-command --example "value"
```
