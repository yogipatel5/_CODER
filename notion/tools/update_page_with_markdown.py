...
class UpdatePageWithMarkdownToolInput(BaseModel):
    """Input schema for uploading markdown to Notion."""

    page_id: str = Field(..., description="ID of the target Notion page.")
    markdown_content: str = Field(..., description="Markdown content to upload.")
    title: str = Field(..., description="Required title for the page.")
    replace_content: bool = Field(
        False, description="If True, replaces all existing content on the page. If False, appends content."
    )


class UpdatePageWithMarkdownTool(BaseTool):
    name: str = "Update Page with Markdown Tool"
    description: str = (
        "Tool for updating a Notion page with markdown content. "
        "Converts markdown to Notion blocks and updates the specified page."
    )
    args_schema: Type[BaseModel] = UpdatePageWithMarkdownToolInput
    ...
    def _run(
        ...
        print(f"Debug: Updating page: {page_id}")

        # We should delete the content of the page here first.
        ...
        try:
            ...
            async def process_block_with_children(blocks_to_process, parent_id):
                ...
                # print("Debug: Blocks creation response:", json.dumps(response, indent=2))
            ...
            if title:
                print(f"Debug: Updating page title to: {title}")
                notion.pages.update(page_id, properties={"title": {"title": [{"text": {"content": title}}]}})

            return f"Markdown content successfully uploaded to page '{page_id}'"
            ...
            return {"success": False, "error": f"Error uploading markdown: {str(e)}"}
...
if __name__ == "__main__":
    ...
    tool = UpdatePageWithMarkdownTool()
    try:
        result = tool.run(
            page_id=os.getenv(
                "NOTION_TEST_PAGE_ID",
