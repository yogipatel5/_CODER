...
class GitInfo(TypedDict):
    """Type definition for git repository information"""
...
ActionType = Literal["keep", "remove", "backup"] | str  # For "move" actions
...
def get_organization_recommendation(repo_info: RepoInfo) -> Tuple[ActionType, str]:
    ...
    Returns:
        Tuple of (action, notes) where:
            - action: the recommended action
            - notes: explanation of why the action was chosen
    """
...
def scan_directory() -> List[RepoInfo]:
    ...
    for item in os.listdir(code_dir):
        ...
        if os.path.isdir(full_path):
            ...
            if repo_info["is_git"]:
                ...
                # Manually update git-related fields
                repo_info["remote_url"] = git_info["remote_url"]
                repo_info["branch"] = git_info["branch"]
                repo_info["has_changes"] = git_info["has_changes"]
...
def process_actions(csv_file: str, dry_run: bool = False) -> None:
    ...
    Args:
        csv_file: Path to the CSV file containing actions
