"""
Show all auto-discovered models, tasks, and signals in the pfsense app.
"""
...
class Command(BaseCommand):
    help = "Show all auto-discovered models, tasks, and signals in the pfsense app"

    def _get_models(self):
        """Get all models from the app."""
        app_config = apps.get_app_config("pfsense")
        return app_config.get_models()

    def _get_tasks(self):
        """Get all Celery tasks from the app."""
        import pfsense.tasks

        return [
            (name, obj)
            for name, obj in inspect.getmembers(pfsense.tasks)
            if inspect.isfunction(obj) and hasattr(obj, "delay")
        ]

    def _get_signals(self):
        """Get all signal handlers from the app."""
        import pfsense.signals

        return [
            (name, obj)
            for name, obj in inspect.getmembers(pfsense.signals)
